...
- **Amendment (found during final whole-branch review):** six cross-cutting issues surfaced by reviewing the complete diff rather than one task at a time, all fixed in one follow-up commit: (1) `_chunked_screen` budgeted only the body text, never accounting for `trailing_options` length, so screens with realistic content (e.g. `situation_list` with several ordinarily-titled situations, or `situation_detail` with multiple linked topics) exceeded Africa's Talking's ~182-char cap and silently lost their navigation options — fixed by reserving budget for the trailing block and by making `render_situation_list`/`render_situation_detail`'s item lists length-aware instead of a fixed count; (2) `chunk_text` discarded all newlines (`text.split()` on whitespace, rejoined with spaces), collapsing multi-line content like the emergency contact list into one run-on line — fixed to preserve line structure while still word-wrapping and paginating; (3) an unknown/terminal `session.state` (e.g. a retried request after `goodbye`) crashed with `KeyError` instead of restarting gracefully, and the view had no exception guard, risking a raw Django 500 sent to Africa's Talking — fixed with a state/`is_active` check that resets to `language_select`, plus a `try/except` in the view returning a graceful `END`; (4) no idempotency guard against Africa's Talking's retry-on-timeout behavior meant a replayed request could double-advance the menu — fixed by caching the last processed `text`/response pair on `UssdSession` (new fields + migration) and returning the cached response on an exact replay; (5) `RightsTopic.risk_level` (added earlier) was invisible in `RightsTopicAdmin` and `RightsTopicSerializer`, blocking content review and any future non-USSD channel from implementing the same gate — added to both, mirroring `Situation`'s existing pattern; (6) **product-safety decision, confirmed with the project owner**: the safety gate now fires on `risk_level` alone rather than requiring `risk_level` AND an active `SafetyResponse` — failing closed (falling back to the topic's summary as the warning message, which `render_safety_gate` already did) rather than silently skipping the gate when a content editor marks a topic high-risk but forgets to attach a warning.
- **Amendment (found during Task 8 review):** `handler.py` stores the invalid-attempt counter as an `"attempts"` key directly inside `session.context`, but six transition functions advance to the next text chunk via `{**session.context, "chunk_index": chunk_index + 1}` — spreading the *entire* prior context, including a stale `"attempts"` key. That means a successful "More" navigation after an invalid attempt does not clear the counter, so non-consecutive invalid attempts (interleaved with valid chunk-advance presses) can still add up to 3 and wrongly end the session, violating the "3 *consecutive*" requirement below. Fix (contained entirely in `handler.py`, no `menus.py` change needed): before persisting `next_context` on the valid-transition path, strip any `"attempts"` key from it, e.g. `next_context = {k: v for k, v in next_context.items() if k != "attempts"}`.
